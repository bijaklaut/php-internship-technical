<?php foreach ($barangs as $barang) : ?>
   <label for="<?= $barang["kode_barang"] ?>" class="flex justify-between items-center p-1 text-sm">
      <input type="checkbox" name="barang" id="<?= $barang["kode_barang"] ?>" value="<?= $barang["kode_barang"] ?>" class="hidden peer">
      ...
      <div class="h-5 w-5 rounded-full border flex items-center justify-center text-white border-black/40 transition-all duration-300 peer-checked:bg-sky-500">
         <svg class="stroke-current h-4 w-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
